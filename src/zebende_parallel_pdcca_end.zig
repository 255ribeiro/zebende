...
export fn p_dcca(
    ...
    for (0..tws_len, tws[0..]) |n_index, n| {
        ...
        { // Allocation block
            ...
            { // handlers detrend scope --start
                ...
                print("detrended threads joined for time scale no. {} of size {} \n", .{ n_index, n });
...

            {
                ...
                var hand_dfa_dcca = allocator.alloc(std.Thread, operator_alloc.len) catch |err| {
        ...
        } // det mat allocation end

        // Allocation free
    } // Time scale loop -- end
    print("{}", .{@TypeOf(P_DCCA_arr)});
    // // p_dcca calc

    // var hand_pdcca = allocator.alloc(std.Thread, DCCA_of_n_rows * tws_len) catch unreachable;

    // for (0..hand_pdcca.len) |data_index| {
    //     const n_index = data_index % tws_len;
    //     const dcca_pair = @divFloor(data_index, tws_len);
    //     hand_pdcca[data_index] = Thread.spawn(.{}, pDccaSimpleOutPrl, .{
    //         // mat imputs
    //         &F_DFA_arr[n_index].*,
    //         &DCCA_arr[n_index].*,
    //         &DCCA_of,
    ...
    //         &P_DCCA_arr[n_index].*,
    //     }) catch |err| {
    //         print("Error spawning P_DCCA Thead no. {} - Error {}", .{ data_index, err });
    //         break;
    //     };
    //     for (hand_pdcca) |h| h.join();
    // }

    print("P_DCCA calculated for {} DCCA combinations in {} time scales\n", .{ DCCA_of_n_rows, tws_len });
} // function --end
