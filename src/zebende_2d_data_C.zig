...
const MainOperator = struct {
    ...
    fn shiftWindow(self: *MainOperator, n: usize, win_start: usize, F_DFA_ptr: [*c]f64) void {
        ...
        if (win_start != 0) {
            ...
            self.series_window = self.data[win_start + n][0..self.left_y.len];

            for (0..self.left_y.len) |sr_index| {
                self.current.sum_y[sr_index] = self.current.sum_y[sr_index] - self.left_y[sr_index] + self.series_window[sr_index];
                self.current.sum_xy[sr_index] = self.current.sum_xy[sr_index] - (self.left_x * self.left_y[sr_index]) + (self.time_window[n] * self.series_window[sr_index]);
            }
        } else { // ( win_start == 0)
            ...
            for (self.previous.window_len..self.current.window_len) |i| {
                ...
                self.series_window = self.data[win_start + i][0..self.left_y.len];

                for (0..self.left_y.len) |sr_index| {
                    self.previous.sum_y[sr_index] += self.series_window[sr_index];
                    self.previous.sum_xy[sr_index] += self.time_window[i] * self.series_window[sr_index];
                }
            }
        ...
        for (0..self.left_y.len) |sr_index| {
            ...
            for (self.time_window, 0..) |t, j| {
                self.series_window = self.data[win_start + j][0..self.left_y.len];

                self.det_mat[sr_index][j] = self.series_window[sr_index] - (self.slope[sr_index] * t + self.inter[sr_index]);
        ...
        self.left_y = self.data[win_start][0..self.left_y.len];
    }
