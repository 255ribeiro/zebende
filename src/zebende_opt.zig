...
export fn p_dcca(
    ...
    for (0..data_count) |sr_indexex| {
        series_array[sr_indexex] = SerieContainer.init(allocator);
        series_array[sr_indexex].serie = data[(sr_indexex * data_len)..((sr_indexex * data_len) + data_len)];
        series_array[sr_indexex].det_row = &det_mat[sr_indexex];
    }
